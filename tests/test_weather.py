...
from sandkings import (
    COLD_DURATION, FLOOD_DAMAGE, FLOOD_WIDTH, SandKing,
...
def test_flood_drowns_extinguishes_and_silts():
    sim = make_sim()
    colony = sim.colonies[0]
    exposed, sheltered = surface_pair(sim, colony, x=10, y=18)
    # a fire in the flood's path
    fz = sim.world.surface_z(10, 5)
    sim.world.voxels[10, 5, fz] = VoxelType.CROP.value
    sim._ignite((10, 5, fz))
    assert sim.fires
    sim.step_count = 2
    sim.flood_edge = 1
    sim.flood_head = 9
    sim.flood_until = sim.step_count + 200
    random.seed(4)
    hp = exposed.health
    ...
    for _ in range(8):
        ...
        sim.step_count += 1
    assert exposed.health < hp, "the surge drowns the exposed"
    assert sheltered.health == sheltered.max_health
    assert (10, 5, fz) not in sim._fires(), "water beats fire"
    tilled_after = int((sim.world.voxels == VoxelType.TILLED.value).sum())
    assert tilled_after > tilled_before, "the silt tills the sand"


def test_flood_ends_when_band_exits():
    sim = make_sim()
    sim.step_count = 2
    sim.flood_edge = 1
    sim.flood_head = sim.world.width + FLOOD_WIDTH  # already past the edge
    sim.flood_until = sim.step_count + 100
    sim._weather_tick(season=0)
    assert sim.flood_until == 0
...
def test_weather_state_pickles_and_old_checkpoints_clear_skied():
    ...
    sim.cold_until = 60
    sim.flood_until = 70
    sim.flood_edge, sim.flood_head = 1, 5
    revived = pickle.loads(pickle.dumps(sim))
    assert revived.hail_until == 50 and revived.flood_head == 5
    # pre-weather checkpoint: strip the attrs, sim must still step
    for attr in ('hail_until', 'cold_until', 'flood_until',
                 'flood_edge', 'flood_head'):
